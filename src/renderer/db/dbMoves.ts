...
export const getMovimientosByCcIdAndDate = async (id: number, fecha: Date) => {
  const res = await axios.get(`${API_URL}/cc/${id}/dia`, {
    params: { fecha: fecha.toISOString() }
  });
  return res.data;
};

export const getLatestMovesByCcId = async (id: number) => {
  const res = await axios.get(`${API_URL}/cc/${id}/dia`)
  return res.data;
}
